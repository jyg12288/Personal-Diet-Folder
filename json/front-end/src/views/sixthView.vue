    ...
    <v-app>
        <v-container>
            ...
            <div style="text-align: center;padding-top: 200px;">
                <b><p style="font-size: 25px; padding-bottom: 20px;">귀하의 체중은 얼마인가요?</p></b>
                <p style="font-size: 15px; color:gray">
                    일일 칼로리 목표를 정확히 계산하려면<br/>
                    체중 정보가 필요합니다. (kg 기준)
                </p>
                <div class="inputBox shadow">
                    <input type="text" v-model="user.weight" placeholder="     체중을 입력해주세요   ">
                </div>
            </div>
            ...
            <div style="text-align: center; padding-top: 80px;">
                ...
                <router-link to="/sevethView" style="text-decoration:none;">
                    <button type="button shadow"
                    id ="target_btn"
                    :disabled="user.weight.length<1"
                    @click="Modify4"
...
export default {
    ...
    methods:{
        Modify4:function(){
            this.$http.post('/api/users/Modify4', { 
            user: this.user
        })
        .then((res) => {
