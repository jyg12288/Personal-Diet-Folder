    ...
    <v-app>
        <v-container>
            <div style="padding-top:10px;">
                <progress id="progress" value="64" min="0" max="100"></progress>
            </div>
            <div style="text-align: center;padding-top: 200px;">
                <b><p style="font-size: 25px; padding-bottom: 20px;">귀하의 키는 얼마인가요?</p></b>
                <p style="font-size: 15px; color:gray">
                    일일 칼로리 목표를 정확히 계산하려면<br/>
                    키 정보가 필요합니다. (cm 기준)
                </p>
                <div class="inputBox shadow">
                    <input type="text" v-model="user.height" placeholder="     키를 입력해주세요   ">
                </div>
            </div>
            ...
            <div style="text-align: center; padding-top: 80px;">
                ...
                <router-link to="/sixthView" style="text-decoration:none;">
                    <button type="button shadow"
                    id ="target_btn"
                    :disabled="user.height.length<1"
                    @click="Modify3"
...
export default {
    ...
    methods:{
        Modify3:function(){
            this.$http.post('/api/users/Modify3', { 
            user: this.user
        })
        .then((res) => {
